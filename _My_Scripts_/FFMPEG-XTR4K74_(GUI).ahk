global RegKey:= "HKEY_CURRENT_USER\Software\WMP_MATT"
goto,init
...
if !Secs2Sample_Start { ;Get start-time.
	if Secs2Sample_Start:= Round(controls.currentPosition)
		...
		Try,Sample_start_min:= Floor(Secs2Sample_Start/60)
	}
	...
	if ANACONDA {
		...
		if instr(DefaultChoice,c1) 
			...
			Choices=%c1%|%c2%|%c3%|%c4%|
		else,Choices=%c1%||%c2%|%c3%|%c4% ;If no default found, default="Extract region".
	} else {
		if instr(DefaultChoice,c1)
			Choices=%c1%||%c2%
		else,if instr(DefaultChoice,c2)
	...
	If(DefaultChoice="Extract region") {
		...
		else,GuiControl,Text,trim2end , % "Xtract - end"
	}
	...
	else,gui,xtract:add,checkbox,%checkbox_Height% vtrim2end +disabled,% del2endstr
	if trim2enddef
	...
	else,gui,xtract:add,checkbox,%checkbox_Height% vReplaceOriginal, Replace original
	...
	gui,xtract:Show,w151 Center,% Script_title
	gui,Submit,NoHide
	sleep,1000
	ActiveHwnd:= WinExist("A")
	Choice_old:= Choice, trim2end_old:= trim2end,PreserveFileDate_old:= PreserveFileDate
	, ReplaceOriginal_old:= ReplaceOriginal, Split_old:= Split
	loop {
		if((choice!=choice_old)||(trim2end!=trim2end_old)||(PreserveFileDate!=PreserveFileDate_old)
		||(ReplaceOriginal!=ReplaceOriginal_old)||(Split!=Split_old)) {
				if(Split!=Split_old) && Split {
					GuiControl,Text,Split,% SplitStr
					GuiControl,enable,trim2end
				}
				if(trim2end!=trim2end_old) && trim2end {
			...
			} else,GuiControl,enable,ReplaceOriginal

			If(choice!=choice_old)&&(choice="Extract region") {
				GuiControl,Enable,trim2end
				if Sample_start_Sec
					GuiControl,Text,trim2end,% "Xtract " . Sample_start_min . ":" . Sample_start_Sec . " - end"
				else,GuiControl,Text,trim2end,% "Xtract - end"
			...
			If(choice!=choice_old)&&(choice="Delete region") {
				GuiControl,Enable,trim2end
				if Sample_start_Sec
					GuiControl,Text,trim2end, % "Crop " . Sample_start_min . ":" . Sample_start_Sec . " - end"
				else,GuiControl,Text,trim2end,% "Crop - end"
				GuiControl,Text,Split,% SplitStr
				GuiControl,enable,Split
			}	
			if trim2end
...
settimer,Xtract_Cleanup,-1 ; settimer,Icon_Alternate,off ;
gui,destroy
return,
...
Secs2Sample_Start:= ""
switch,Choice {
	case "Extract Region"	: process_Action:= "-t",	process_Type:= "Extract"
	case "Delete Region"	: process_Action:= "-to",	process_Type:= "Trimmed"
	case "Extract Vox" 		: process_Action:= "-t", 	process_Type:= "Extract Vox" 
	case "Extract Inst" 	: process_Action:= "-t",	process_Type:= "Extract Instrumental"
}

Output_Prefix := (OutNameNoExt . " - " . process_Type), Output_FullUnc:= (OutDir "\" Output_Prefix "." OutExtension)

File_Numbering:
if (n:= fileexist(Output_FullUnc)) {
	n:= 1, RegXPos:= 1, Match:= ""
	while,RegXPos:= regexmatch(Output_Prefix,needL,Match,RegXPos)
		File_Num:= Match, RegXPos:= 666
	(!File_Num?	File_Num:=1 : File_Num++) 
	Output_FullUnc=%OutDir%\%Output_Prefix% %File_Num%.%OutExtension%
} else,Output_FullUnc=%OutDir%\%Output_Prefix%.%OutExtension%
if FileExist(Output_FullUnc) { ; Check_Folder; SplitPath,Output_FullUnc ,,,,Output_Prefix
...
If(choice="Delete region") {
	...
	if !trim2end {
		...
		Try,fileDelete,% extract_data_1 ;FileRecycle,%FullPath%
		Try,fileDelete,% extract_data_2
		Try,fileDelete,% extract_data_Concat
	}
	...
	(trim2end? Output_FullUnc:= extract_data_1)
	A:="",B:=""
	A=%extract_starttime%
	B=%extract_endtime%
	if Split
		goto,dr ;run,%comspec% /c ffmpeg -i "%FullPath%" -ss %A% %Process_Action% %B% -c:v copy -c:a copy "%Output_FullUnc%",,
	else {
		...
		if (choice="Extract vox"||choice="Extract inst" ) {
			...
			;	else {
					...
					if !fileexist(Main_Result_WAV) { ; check spleeter files have appeared
						sleep,% S
						goto,Check_Output_2
					} else,msgbox,%Main_Result_WAV% exist 
					fileMove,%Main_Result_WAV%,%Output_FullUnc%
					if errorlevel
						msgbox,% errorlevel " err5"
					Check_Output_Moved:
...
Reg_Read:
regRead,DefaultChoice,% RegKey,	Extractor Default
regRead,Choice,% RegKey,		Extractor Default
regRead,Splitdef,% RegKey,		Split Default,
regRead,Trim2enddef,% RegKey,	Trim2end Default
regRead,PreserveDef,% RegKey,	PreserveFileDate
regRead,ReplaceDef,% RegKey,	ReplaceOriginal
return,

Reg_Write:
regWrite,REG_SZ,% RegKey,Extractor Default,	% Choice
regWrite,REG_SZ,% RegKey,Split Default,		% Split
regWrite,REG_SZ,% RegKey,Trim2end Default,	% Trim2end
regWrite,REG_SZ,% RegKey,PreserveFileDate,	% PreserveFileDate
regWrite,REG_SZ,% RegKey,ReplaceOriginal,	% ReplaceOriginal
return,

Errormsg:
tooltip,bad bad WMP...`nrestarting
sleep,3000
reload,
